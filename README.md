...
#### Prerequisites
- **Node.js** (version 22 or above recommended, see `.nvmrc`)
- **npm** or **yarn** package manager
- **Spotify Developer account** for API access (to get your `client_id`. This project uses the PKCE auth flow of Spotify, so you don't need the `client_secret`).
...
3. **Set up environment variables**
   ...
   ```
   Then replace the `YOUR_SPOTIFY_CLIENT_ID_HERE` placeholder with your obtained `client_id`.
...
| `npm run preview:https` | Serves the production build with HTTPS |
| `npm test` | Launches the test runner (Vitest) |
| `npm run lint` | Checks the code for linting errors |
| `npm run generate-certs` | Generates self-signed certificates for local HTTPS |
...
│   ├── data/               # Configuration and app data
...
│   ├── paintings/          # Title image generation
│   ├── routes/             # Page-level components
│   ├── services/           # Pdf generation and spotify-related logic
│   ├── types/              # TypeScript type definitions
│   └── worker/             # Web workers (e.g., PDF generation)
